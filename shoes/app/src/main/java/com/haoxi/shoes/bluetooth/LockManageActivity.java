package com.haoxi.shoes.bluetooth;
...
import com.haoxi.shoes.base.ShoesApp;
import com.haoxi.shoes.bluetooth.utils.HexUtils;
import com.haoxi.shoes.bluetooth.utils.MPermissionsActivity;
import com.haoxi.shoes.bluetooth.utils.SampleGattAttributes;
...
public class LockManageActivity extends MPermissionsActivity implements View.OnClickListener {
    ...
    protected void onDestroy() {
        ...
        ShoesApp.getInstance().getBluetoothLeService().close();
    }
    ...
    private void initWidget() {
        ...
        if (!TextUtils.isEmpty(address)) {
            ...
            ShoesApp.getInstance().getBluetoothLeService().connect(address);
        }
    ...
    private void parseData(String value) {
        ...
        byte[] x = new byte[16];
        Log.e(LockManageActivity.class.getSimpleName(), "value:" + value);
        if(value.length()<32) {
            System.arraycopy(values, 0, x, 0, value.length()/2);
        }else{
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.bt_open://开锁
                ...
                ShoesApp.getInstance().getBluetoothLeService().writeCharacteristic(sendDataBytes);
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 0:
                    ShoesApp.getInstance().getBluetoothLeService().writeCharacteristic(gettoken);
                    break;
                case 1://获取电量
                    byte[] batteryBytes = {0x02, 0x01, 0x01, 0x01, token[0], token[1], token[2], token[3], 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
                    ShoesApp.getInstance().getBluetoothLeService().writeCharacteristic(batteryBytes);
                    break;
                case 2://开锁
                    sendDataBytes = new byte[]{0x05, 0x01, 0x06, SampleGattAttributes.password[0], SampleGattAttributes.password[1], SampleGattAttributes.password[2], SampleGattAttributes.password[3], SampleGattAttributes.password[4], SampleGattAttributes.password[5], token[0], token[1], token[2], token[3], 0x00, 0x00, 0x00};
                    ShoesApp.getInstance().getBluetoothLeService().writeCharacteristic(sendDataBytes);
